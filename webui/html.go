...
<div id="page-scan" class="page active">
  ...
  <!-- Real-Time Charts -->
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px" id="chartsSection">
    <div style="background:var(--bg);border:1px solid var(--bd);border-radius:var(--rad-sm);overflow:hidden">
      <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;background:var(--bg2);border-bottom:1px solid var(--bd)">
        <span style="font-family:var(--font-mono);font-size:9px;letter-spacing:2px;color:var(--dim)">LATENCY LIVE</span>
        <span style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:var(--y)" id="chartLatVal">—</span>
      </div>
      <div style="padding:8px 12px 10px"><canvas id="chartLatency" height="70" style="display:block;width:100%"></canvas></div>
    </div>
    <div style="background:var(--bg);border:1px solid var(--bd);border-radius:var(--rad-sm);overflow:hidden">
      <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;background:var(--bg2);border-bottom:1px solid var(--bd)">
        <span style="font-family:var(--font-mono);font-size:9px;letter-spacing:2px;color:var(--dim)">PASS RATE</span>
        <span style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:var(--g)" id="chartPassVal">0%</span>
      </div>
      <div style="padding:8px 12px 10px"><canvas id="chartPass" height="70" style="display:block;width:100%"></canvas></div>
    </div>
    <div style="background:var(--bg);border:1px solid var(--bd);border-radius:var(--rad-sm);overflow:hidden">
      <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;background:var(--bg2);border-bottom:1px solid var(--bd)">
        <span style="font-family:var(--font-mono);font-size:9px;letter-spacing:2px;color:var(--dim)">SCAN SPEED</span>
        <span style="font-family:var(--font-mono);font-size:14px;font-weight:700;color:var(--p)" id="chartSpeedVal">0 ip/s</span>
      </div>
      <div style="padding:8px 12px 10px"><canvas id="chartSpeed" height="70" style="display:block;width:100%"></canvas></div>
    </div>
    <div style="background:var(--bg);border:1px solid var(--bd);border-radius:var(--rad-sm);overflow:hidden">
      <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;background:var(--bg2);border-bottom:1px solid var(--bd)">
        <span style="font-family:var(--font-mono);font-size:9px;letter-spacing:2px;color:var(--dim)">LAT DIST</span>
        <span style="font-family:var(--font-mono);font-size:9px;color:var(--o)">HISTOGRAM</span>
      </div>
      <div style="padding:8px 12px 10px"><canvas id="chartHist" height="70" style="display:block;width:100%"></canvas></div>
    </div>
  </div>
...
// ══ REAL-TIME CHARTS ══
class MiniLineChart {
  constructor(id, color, maxPts=50) {
  ...
  _resize() {
    ...
    const w = this.canvas.parentElement.offsetWidth || 200;
    const h = parseInt(this.canvas.getAttribute('height')) || 70;
  ...
  draw() {
    ...
    const {ctx, w, h, data, maxPts, color} = this;
    ctx.clearRect(0, 0, w, h);
    const mn = Math.min(...data) * 0.9, mx = Math.max(...data) * 1.1 || 1;
    const xs = w / (maxPts - 1), ys = (h - 16) / (mx - mn);
    const ox = (maxPts - data.length) * xs;
    // grid
    ctx.strokeStyle = 'rgba(30,45,70,.5)'; ctx.lineWidth = 1;
    for (let i = 0; i <= 3; i++) { const y = 6 + i * (h - 16) / 3; ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(w, y); ctx.stroke(); }
    // fill
    ctx.beginPath();
    data.forEach((v, i) => { const x = ox + i * xs, y = h - 8 - (v - mn) * ys; i ? ctx.lineTo(x, y) : ctx.moveTo(x, y); });
    const lx = ox + (data.length - 1) * xs;
    ctx.lineTo(lx, h); ctx.lineTo(ox, h); ctx.closePath();
    ...
    // dot
    const lv = data[data.length - 1], ly = h - 8 - (lv - mn) * ys;
...
class MiniHistChart {
  ...
  push(ms) {
    const i = Math.min(Math.floor(ms / 300), 9);
    this.buckets[i]++;
    this.draw();
  }
  draw() {
    if (!this.ctx) return;
    const {ctx, w, h, buckets} = this;
    ctx.clearRect(0, 0, w, h);
    const mx = Math.max(...buckets) || 1;
    const bw = (w - 10) / buckets.length;
    const cls = ['#00ffaa','#00ffaa','#aaff00','#ffd700','#ffd700','#ff9900','#ff7700','#ff4400','#ff2d6b','#ff2d6b'];
    buckets.forEach((v, i) => {
      const x = 5 + i * bw, bh = (v / mx) * (h - 16), y = h - 10 - bh;
      ctx.fillStyle = cls[i] + '25'; ctx.fillRect(x+1, y, bw-3, bh);
      ctx.fillStyle = cls[i]; ctx.fillRect(x+1, y, bw-3, 2);
      ctx.shadowBlur = 4; ctx.shadowColor = cls[i]; ctx.fillRect(x+1, y, bw-3, 2); ctx.shadowBlur = 0;
    });
  }
}

// Init charts
let _chartLat, _chartPass, _chartSpeed, _chartHist;
function initCharts() {
  _chartLat   = new MiniLineChart('chartLatency', '#ffd700');
  _chartPass  = new MiniLineChart('chartPass',    '#00ffaa');
  _chartSpeed = new MiniLineChart('chartSpeed',   '#c060ff');
  _chartHist  = new MiniHistChart('chartHist');
}

function pushChartData(latencyMs, passRatePct, speedIPS) {
  if (latencyMs  != null && _chartLat)   _chartLat.push(latencyMs);
  if (passRatePct!= null && _chartPass)  _chartPass.push(passRatePct);
  if (speedIPS   != null && _chartSpeed) _chartSpeed.push(speedIPS);
  if (latencyMs  != null && _chartHist)  _chartHist.push(latencyMs);
  if (_chartLat  && latencyMs  != null) document.getElementById('chartLatVal') && (document.getElementById('chartLatVal').textContent = Math.round(latencyMs) + 'ms');
  if (_chartPass && passRatePct!= null) document.getElementById('chartPassVal') && (document.getElementById('chartPassVal').textContent = passRatePct.toFixed(1) + '%');
  if (_chartSpeed&& speedIPS   != null) document.getElementById('chartSpeedVal') && (document.getElementById('chartSpeedVal').textContent = speedIPS.toFixed(1) + ' ip/s');
}
