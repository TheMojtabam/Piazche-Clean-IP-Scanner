...
function renderHealth(entries){
  ...
  el.innerHTML='';
  if(!entries.length){
    el.innerHTML='<div style="color:var(--dim);font-size:12px;text-align:center;padding:32px">هیچ IP ای در مانیتور نیست.<br>از صفحه Results دکمه ♡ رو بزن</div>';
    return;
  }
  ...
  entries.forEach(e=>{
    ...
    const lastChk=e.lastCheck?new Date(e.lastCheck).toLocaleTimeString():'—';
    ...
    div.innerHTML='<span style="color:'+col+';font-size:18px;flex-shrink:0">'+icon+'</span>'+
      '<div style="flex:1">'+
        ...
        '<div style="font-family:var(--font-mono);font-size:10px;color:var(--dim);margin-top:3px">'+
          'lat: <span style="color:var(--y)">'+lat+'</span> · uptime: <span style="color:var(--g)">'+uptime+'</span> · '+lastChk+
        '</div>'+
      '</div>'+
