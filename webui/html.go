...
<div id="page-scan" class="page active">
  <div class="phd">
    ...
    <div class="phd-r">
      ...
      <button class="btn btn-warn" id="btnPause" onclick="pauseScan()" style="display:none">⏸ Pause</button>
    </div>
  </div>
...
<div id="page-config" class="page">
  ...
  <div class="card">
    ...
    <div class="card-bd">
      ...
      <div id="fragAutoInfo" style="display:none;background:var(--gd);border:1px solid var(--g2);border-radius:var(--rad-xs);padding:10px;font-size:11px;color:var(--g);font-family:var(--font-mono);margin-top:6px">
        ✓ Auto mode: اسکنر همه حالات رو تست می‌کنه و بهترین ترکیب رو انتخاب میکنه<br>
        zones: tlshello · 1-3 · 1-5 · 1-10 · random
      </div>
      <div style="font-size:10px;color:var(--dim);font-family:var(--font-mono);margin-top:4px" id="fragManualInfo">
        Manual: مقادیر رو خودت تنظیم کن. Auto: اسکنر خودش بهترین رو پیدا میکنه.
      </div>
    </div>
  </div>
...
function setStatus(st,phase){
  ...
  // Pause فقط در فاز 1 — فاز 2 pause نمیشه
  const isP1=st==='scanning'&&(phase===''||phase==='phase1');
  const isPaused=st==='paused';
  document.getElementById('btnPause').style.display=(isP1||isPaused)?'':'none';
  ...
  tbody.innerHTML=p2Results.map((r,i)=>{
    ...
    return '<tr class="'+(r.Passed?'pass-row':'fail-row')+'">'+
      ...
      '<td><div style="display:flex;gap:3px">'+
        '<button class="copy-btn" data-ip="'+r.IP+'" data-action="copyip" title="Copy IP">⎘</button>'+
        '<button class="copy-btn" data-ip="'+r.IP+'" data-action="copyvless2" title="Copy vless link">⬡</button>'+
      '</div></td></tr>';
  }).join('');
}
