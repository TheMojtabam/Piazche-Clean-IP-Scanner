...
func saveStateToDisk(proxyJSON, scanJSON, rawURL string, templates []config.ConfigTemplate) {
	data, _ := json.MarshalIndent(persistedState{ProxyConfig: proxyJSON, ScanConfig: scanJSON, Templates: templates, RawURL: rawURL}, "", "  ")
	os.WriteFile(configPersistPath(), data, 0644)
}

func loadStateFromDisk() (proxyJSON, scanJSON, rawURL string, templates []config.ConfigTemplate) {
	...
	return ps.ProxyConfig, ps.ScanConfig, ps.RawURL, ps.Templates
}
...
func NewServer(port int) *Server {
	...
	proxyJSON, scanJSON, rawURL, savedTemplates := loadStateFromDisk()
...
func (s *Server) handleConfigParse(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasPrefix(input, "vless://") || strings.HasPrefix(input, "vmess://") || strings.HasPrefix(input, "trojan://") {
		...
		s.state.mu.Unlock()
		go saveStateToDisk(proxyJSON, scanJSON, rawURL, templates)
	}
...
func (s *Server) handleTemplateDelete(w http.ResponseWriter, r *http.Request) {
	...
	// سیو روی دیسک
	go saveStateToDisk(proxyJSON, scanJSON, rawURL2, templates2)
