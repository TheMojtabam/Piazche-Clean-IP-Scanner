...
type Config struct {
	...
	Phase3   Phase3Config   `json:"phase3,omitempty"`
...
// Phase3Config تنظیمات فاز سوم — تست سرعت جداگانه
type Phase3Config struct {
	Enabled     bool    `json:"enabled"`
	DownloadURL string  `json:"downloadUrl"`
	UploadURL   string  `json:"uploadUrl"`
	MinDLMbps   float64 `json:"minDlMbps"`
	MinULMbps   float64 `json:"minUlMbps"`
	TestUpload  bool    `json:"testUpload"`
}

// MonitorConfig تنظیمات health monitor
type MonitorConfig struct {
	Enabled             bool `json:"enabled"`
	CheckIntervalMinutes int  `json:"checkIntervalMinutes"` // default: 3
	TrafficDetect       bool `json:"trafficDetect"`         // detect data passing without speed test
}
...
type HealthEntry struct {
	...
	LatencyHistory []int64      `json:"latencyHistory"` // آخرین ۵۰ latency (ms) — 0 یعنی fail
	CheckTimes     []int64      `json:"checkTimes"`     // timestamp هر check (unix ms)
	GeoInfo        *GeoInfo     `json:"geoInfo,omitempty"`
}

// GeoInfo اطلاعات جغرافیایی یه IP
type GeoInfo struct {
