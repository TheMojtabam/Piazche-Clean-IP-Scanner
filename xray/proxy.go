...
// EstimateBandwidth تخمین سریع پهنای باند بدون دانلود فایل بزرگ
// از اندازه response و زمان اتصال استفاده می‌کنه
// نتیجه تقریبیه ولی سریعه (بدون 5MB دانلود)
func EstimateBandwidth(ctx context.Context, socksPort int, testURL string, timeout time.Duration) (estimatedMbps float64, err error) {
	parsedURL, err := url.Parse(testURL)
	if err != nil {
		return 0, fmt.Errorf("invalid URL: %w", err)
	}
	client, err := makeSOCKSClient(socksPort, parsedURL.Hostname(), timeout, true)
	if err != nil {
		return 0, err
	}

	// چند سایز مختلف دانلود می‌کنیم و slope رو حساب می‌کنیم
	// این روش دقیق‌تر از یه request تنهاست
	sizes := []int{10000, 50000, 100000} // 10K, 50K, 100K
	var measurements []struct{ bytes int; ms float64 }

	for _, sz := range sizes {
		u := fmt.Sprintf("https://speed.cloudflare.com/__down?bytes=%d", sz)
		...
		if n > 0 && elapsed > 0 {
			measurements = append(measurements, struct{ bytes int; ms float64 }{int(n), elapsed})
		}
	}

	if len(measurements) == 0 {
		return 0, fmt.Errorf("no measurements")
	}

	// میانگین weighted: بزرگ‌ترین sample دقیق‌تره
	var totalBytes float64
	var totalMs float64
	for _, m := range measurements {
		totalBytes += float64(m.bytes)
		totalMs += m.ms
	}
	if totalMs <= 0 {
		return 0, fmt.Errorf("zero elapsed")
	}
	bps := totalBytes / (totalMs / 1000)
	return bps / 1024 / 1024 * 8, nil
}
