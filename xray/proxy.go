...
func TestPacketLoss(ctx context.Context, socksPort int, testURL string, count int, timeout time.Duration) (lossPercent float64, err error) {
	if count <= 0 {
		count = 5
	}
	...
	client := &http.Client{
		Transport: transport,
		Timeout:   timeout / time.Duration(count),
		CheckRedirect: func(req *http.Request, via []*http.Request) error {
			return http.ErrUseLastResponse
		},
	}
	...
	for i := 0; i < count; i++ {
		select {
		case <-ctx.Done():
			// remaining are considered lost
			lost += count - i
			break
		default:
		}

		req, e := http.NewRequestWithContext(ctx, "GET", testURL, nil)
		if e != nil {
			lost++
			continue
		}
